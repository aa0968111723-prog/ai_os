...
2. **手機上的「Safari」和「加到主畫面的 App」是兩個獨立空間**（iOS 的限制），同一支手機會出現兩台裝置。UI 標籤要能區分：「iPhone・Safari」vs「iPhone・Safari・主畫面」（保留瀏覽器名——不知道是哪個瀏覽器裝的，就不知道要去哪裡移除）。

裝置命名（系統／瀏覽器／機型的判定與翻譯）住在 `shared/deviceNaming.ts`，前後端共用同一份：通知設定頁的三份清單（已連結裝置／登入裝置／信任裝置）與驗證信必須叫同一台手機同一個名字，否則使用者不敢按移除。機型代碼會翻成行銷名（`SM-S928B` → `Samsung Galaxy S24 Ultra`），查不到的代碼原樣顯示——猜錯機型比不寫更糟。
