...
> 唯一端點數:**251**|連通:**219**|取消未確認:**32**|404 不存在:**0**|403 無權限:**0**|暫時性:**0**
...
| `fal-ai/flux-2` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
...
| `fal-ai/imagen4/preview/ultra` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/imagen4/preview/fast` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/flux-2-pro/edit` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/nano-banana/edit` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
...
| `openai/gpt-image-2/edit` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/flux/dev/redux` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/bria/background/replace` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/image-editing/object-removal` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/bria/product-shot` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/image-editing/photo-restoration` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
...
| `fal-ai/minimax/hailuo-2.3/standard/text-to-video` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/kling-video/v2.6/pro/text-to-video` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/minimax/hailuo-02/standard/text-to-video` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/minimax/video-01-director` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `wan/v2.6` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/wan/v2.2-a14b/text-to-video/lora` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `wan/v2.6/image-to-video` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/wan/v2.2-a14b/image-to-video` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/luma-dream-machine/ray-2-flash/modify` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
| `fal-ai/elevenlabs/sound-effects/v2` | 200 | 空輸入已被受理,但取消未確認(cancel → HTTP 400:{"status":"ALREADY_COMPLETED"});需到 Fal request history 核對 |
...
| 端點 | 狀態 | HTTP | 影響的模型 |
|---|---|---|---|
| `fal-ai/flux-2-pro` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/flux-2/pro |
| `fal-ai/qwen-image-2/pro/text-to-image` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/qwen-image-2/pro/text-to-image |
| `openai/gpt-image-2` | ✅ 連通(誤排佇列已取消) | 200 | openai/gpt-image-2 |
...
| `fal-ai/ideogram/v3` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/ideogram/v3 |
| `fal-ai/flux/schnell` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/flux/schnell |
| `fal-ai/qwen-image-2/text-to-image` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/qwen-image-2/text-to-image |
...
| `fal-ai/flux-2` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/flux-2 |
| `fal-ai/flux-pro/v1.1-ultra` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/flux-pro/v1.1-ultra |
...
| `fal-ai/recraft/v4.1/text-to-image` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/recraft/v4.1/text-to-image |
| `fal-ai/nano-banana-pro` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/nano-banana-pro |
| `fal-ai/imagen4/preview/ultra` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/imagen4/preview/ultra |
| `fal-ai/imagen4/preview/fast` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/imagen4/preview/fast |
...
| `fal-ai/sana` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/sana |
...
| `fal-ai/flux-2-pro/edit` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/flux-2/pro/edit |
...
| `fal-ai/flux-pro/kontext` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/flux-pro/kontext |
...
| `fal-ai/nano-banana-pro/edit` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/nano-banana-pro/edit |
| `fal-ai/nano-banana/edit` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/nano-banana/edit |
| `fal-ai/bytedance/seedream/v4/edit` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/bytedance/seedream/v4/edit |
| `fal-ai/bytedance/seededit/v3/edit-image` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/bytedance/seededit/v3/edit-image |
| `fal-ai/qwen-image-2/edit` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/qwen-image-2/edit |
| `openai/gpt-image-2/edit` | ❗ 空輸入已入列但取消未確認 | 200 | openai/gpt-image-2/edit |
| `fal-ai/ideogram/v3/remix` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/ideogram/v3/remix |
| `fal-ai/flux/dev/redux` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/flux/dev/redux |
...
| `fal-ai/bria/expand` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/bria/expand |
| `fal-ai/image-editing/object-removal` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/image-editing/object-removal |
| `fal-ai/bria/product-shot` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/bria/product-shot |
| `fal-ai/codeformer` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/codeformer |
| `fal-ai/image-editing/photo-restoration` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/image-editing/photo-restoration |
| `fal-ai/image-apps-v2/photo-restoration` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/image-apps-v2/photo-restoration |
...
| `clarityai/crystal-upscaler` | ✅ 連通(誤排佇列已取消) | 200 | clarityai/crystal-upscaler |
...
| `fal-ai/ideogram/remove-background` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/ideogram/remove-background |
...
| `fal-ai/kling-video/v2.5-turbo/pro/text-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/kling-video/v2.5-turbo/pro/text-to-video |
...
| `fal-ai/minimax/hailuo-2.3/standard/text-to-video` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/minimax/hailuo-2.3/standard/text-to-video |
| `fal-ai/ltx-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/ltx-video |
| `fal-ai/veo2` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/veo2 |
| `fal-ai/kling-video/v2.6/pro/text-to-video` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/kling-video/v2.6/pro/text-to-video |
...
| `fal-ai/minimax/hailuo-02/standard/text-to-video` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/minimax/hailuo-02/standard/text-to-video |
| `fal-ai/minimax/video-01-director` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/minimax/video-01-director |
| `fal-ai/wan-25/text-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/wan/v2.5/text-to-video |
| `wan/v2.6` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/wan/v2.6/text-to-video |
| `fal-ai/hunyuan-video-v1.5/text-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/hunyuan-video-v1.5/text-to-video |
| `fal-ai/pika/v2.2/text-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/pika/v2.2/text-to-video |
...
| `fal-ai/wan/v2.2-a14b/text-to-video/lora` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/wan/v2.2-a14b/text-to-video/lora |
...
| `fal-ai/kling-video/v2.5-turbo/pro/image-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/kling-video/v2.5-turbo/pro/image-to-video |
| `fal-ai/kling-video/v3/pro/image-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/kling-video/v3/pro/image-to-video |
| `fal-ai/veo3.1/image-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/veo3.1/image-to-video |
...
| `fal-ai/bytedance/seedance/v1.5/pro/image-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/bytedance/seedance/v1.5/pro/image-to-video |
| `fal-ai/luma-dream-machine/ray-2/image-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/luma-dream-machine/ray-2/image-to-video |
| `wan/v2.6/image-to-video` | ❗ 空輸入已入列但取消未確認 | 200 | wan/v2.6/image-to-video |
| `fal-ai/wan/v2.2-a14b/image-to-video` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/wan/v2.2-a14b/image-to-video |
...
| `fal-ai/minimax/video-01/image-to-video` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/minimax/video-01/image-to-video |
| `fal-ai/minimax/video-01-subject-reference` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/minimax/video-01-subject-reference |
...
| `fal-ai/luma-dream-machine/ray-2-flash/modify` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/luma-dream-machine/ray-2-flash/modify |
...
| `fal-ai/workflow-utilities/auto-subtitle` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/workflow-utilities/auto-subtitle |
...
| `fal-ai/elevenlabs/speech-to-text/scribe-v2` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/elevenlabs/speech-to-text#keyterms、fal-ai/elevenlabs/speech-to-text/scribe-v2 |
...
| `fal-ai/elevenlabs/tts/multilingual-v2` | ✅ 連通(誤排佇列已取消) | 200 | fal-ai/elevenlabs/tts/multilingual-v2 |
...
| `fal-ai/elevenlabs/sound-effects/v2` | ❗ 空輸入已入列但取消未確認 | 200 | fal-ai/elevenlabs/sound-effects/v2 |
