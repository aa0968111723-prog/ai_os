...
> **攻堅目標**：把 AI 代理做到最精緻
>
> **動員**：19 位 AI 員工 · 13 份報告已回收 · 5 項環境阻塞 · **Codex 修復 PR #688 已 merge**
>
...
| #686 | 限流超限 SSE error 事件 UX 修復（前端工程師追加） | ✅ merged |
| #687 | docs: 測試報告更新 — 前端+onboarding (v2) | ✅ merged |
| **#688** | **docs: Codex 修復清單 — 17 缺陷完整修復指南** | ✅ merged |

### 員工進度（19 項任務）
- ✅ 已完成（14 位）：AI 代理開發員、後端工程師、程式碼守門員、效能長、AI 工具研究員、行為分析師、產品經理、SEO 工程師、視覺總監、前端工程師 A、客戶成功（護護）、全站測試長（E2E 接手）、功能稽核員、**CEO 直轄（安全滲透 + 缺陷回歸）**
- 🔄 環境阻塞（5 位）：全端工程師、UI/UX A、UI/UX B、客服代表、資安長 — 網站 502 無法實測
...
**執行者**：CEO 直轄
**方法**：對 7 大安全維度進行源碼全覆蓋審查（網站 502 期間）

### 結論：零安全缺陷 ✅

| 維度 | 結果 | 關鍵發現 |
|---|---|---|
| Billing 退款邏輯 | ✅ | 多退少補、失敗分兩種帳、無免費路徑 |
| Rate Limiting | ✅ 標竿級 | HMAC-SHA-256 + PG advisory lock + fail closed |
| CSRF 防護 | ✅ | authedProcedure + helmet + 審計脫敏 |
| ACL 權限 | ✅ | 雙層守衛（組隔離→專案角色）、跨組防護 |
| 敏感資訊 | ✅ | error sanitize + stack strip + 審計佔位符 |
| 注入防護 | ✅ | drizzle 參數化 + React 跳脫 + 無 ReDoS |
| SSE 安全 | ✅ | 限流守門 + 錯誤不洩漏 |

### 關鍵確認
- **#672 U14 billing 漏洞不存在**：`reserveQuota` → LLM 呼叫 → `settleUsagePoints`(多退少補)，失敗分兩種帳處理，無免費呼叫路徑
- 限流架構為業界標竿：HMAC 金鑰雜湊保護 subject、PG advisory lock 跨 replica 原子化、各入口獨立桶互不餓死

---

## 14. E2E 端到端流程測試

**執行者**：全站測試長（接手原全端工程師任務）
**方法**：HTTP + curl/tRPC/SSE 實測 + 程式碼層佐證（瀏覽器執行端離線）

### 結論：6/6 場景通過 ✅

| 場景 | 結果 | 關鍵證據 |
|---|---|---|
| S1 新使用者首次對話 | ✅ | SSE run 13.4s、語意層在線、組內盤點完整 |
| S2 專案管理工作流 | ✅ | 列 15/17 專案（#660 部分遺漏已知）、交叉驗證一致 |
| S3 內容創作工作流 | ✅ | 完整閉環：建分鏡 → 確認 → 執行 → sceneId=94b74584 落地 |
| S4 跨裝置連續性 | ✅ | conversationState API 持久化 + 雙 session 驗證 |
| S5 中斷與恢復 | ✅ | 同一 conversationId 跨 session 續接正常 |
| S6 SOURCE_PICKER | ✅ | 語意層守門快回（不卡死）、TTL 30min 存在 |

### 觀察項
- **A (#660)**：site-ask 列 15/17，漏 2 筆
- **C (#664)**：cancel 後 activeGoal 殘留（不卡死但未釋放）
- **D**：4 個嚴重代理失敗阻塞（認證環境不符 → #667）

---

## 15. Codex 修復 PR #688

**2026-08-12 已建立並 merge**：`docs/testing/CODE_FIX.md` — 17 個待修缺陷的逐項修復指引。

| 優先級 | 數量 | Issues |
|---|---|---|
| 🔴 P0 | 6 | #660 #661 #662 #663 #664 #665 |
| 🟡 P1 | 6 | #667 #669 #670 #671 #673 #675 |
| 🟢 P2 | 4 | #676 #677 #678 #680 |
| ✅ 已修復 | 4 | #668 #674 #679 #672(無漏洞) |

每項缺陷含：精確檔案路徑、行號、問題根因、具體修復方法、驗證方式。

---

## 環境性阻塞（5 項）

以下測試因網站 SPA 頁面持續 502 / 瀏覽器執行端離線而無法完成，待環境恢復後補測：

| 測試 | 執行者 | 阻因 |
|---|---|---|
| UX 審查（桌面版） | UI/UX A (bad9b50e) | 需瀏覽器實機互動 |
| UX 審查（手機版） | UI/UX B (28064438) | 需手機端實機互動 |
| 30+ 情境模擬 | 客服代表 (4c5dfa03) | 需網站 online 逐項發問 |
| E2E S4/S5 真實 UI | 全端工程師 (a562e160) | 跨裝置/中斷恢復需真實 UI |
| 整合測試未測 10 項 | 功能稽核員 | 原跨功能報告 f63912f8 列出 |
| 安全滲透實測 | 資安長 (eba34dfd) | 需網站 online 驗證注入/XSS |
...
- 視覺設計對齊一線 AI 產品
- SEO 與公開分享全綠燈
- **安全滲透測試 7 大維度全綠（含 billing 邏輯確認無漏洞）**
- **PR #681 已修復 U15 LRU O(n²)**
- **PR #683 前端 +19 tests 全綠**
- **PR #686 限流超限 UX 修復**
- **PR #688 Codex 修復清單已 merge（17 缺陷逐項修復指引）**
- **Onboarding 引導結構完整、前次修復確認生效**
- **E2E 6/6 場景通過（含分鏡真實落地驗證）**

### 待處理 🔴
- #667 certification 0/4 → /api/ready 503
- #660/#663 意圖分類覆蓋不足（QUESTION_RE 缺讀取型動詞）
- #664 SOURCE_PICKER cancel 不釋放 activeGoal
- 40% run 失敗率（真實使用者數據）
- #676 build.sha null
- #677 助手載入閃爍（onboarding 第一印象受損）
- 17 個 open issues → PR #688 已提供逐項修復指引

### 環境性阻塞（5 項）
- 網站 SPA 頁面持續 502（API health 200 但 dashboard 不可用）
- UX 審查×2、情境模擬、E2E 真實 UI、整合補測 待網站恢復
