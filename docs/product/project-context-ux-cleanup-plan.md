...
> 狀態：**C0–C3 已實作**（#405–#407 C0–C2；`feat/context-c3-empty-journey` C3）  
...
```
[x] C3.1  「從這裡開始」步驟與 ①②③ 實際完成態對齊（三步：定調／創作／交付；捲動＋揭示）
[x] C3.2  空世界觀：範例卡 CTA「套用後去創作台」一鍵（套用 + reveal studio）
[x] C3.3  TocNav / 階段標題用同一套「定調 → 創作 → 交付」用語，避免 ① 叫法與工作台不一致
```
