...
- [ ] 390×844：載入專案後，不展開任何卡即可在 1～2 滑內看到 `#gen-prompt` 或工作台主輸入
- [ ] 桌機 ≥821px 版面／預設展開與現況一致
- [ ] 留言可開啟 sheet 送出；未讀 badge 可見
- [ ] 摘要 chip 點擊仍可展開並捲到對應區塊
- [ ] 使用者若曾展開並記住偏好，重整後尊重 localStorage（若該區塊有持久化）
- [ ] typecheck + 相關 client 測試通過
...
|----|------|
| **本 PR** | 僅本計畫文件 |
| UX-M1 | Phase 1 實作（收合 + 留言 sheet） |
| UX-M2 | Phase 2 三 tab（可選） |
| UX-M3 | 工作台內減負（可選） |
