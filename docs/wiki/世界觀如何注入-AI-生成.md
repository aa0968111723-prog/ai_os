...
| 規則 | 說明 |
|------|------|
| **風格准單選** | UI 點選即取代；固定顯示「出圖風格」；`selectWorldviewStyle` |
| **調性／主軸順序＝優先** | 陣列第一個＝**主要**；其餘＝備選 |
| **圖影注入** | 風格只取 **1**（出圖／主要）、調性最多 **2**；主軸**不**進圖影 |
| **LLM** | 風格 1、調性 2、主軸 2（硬截斷） |
| **導演／代理／助手** | 標「主要／備選」；超標時附「選項提示」 |
| **UI** | 風格 1（准單選）、調性 ≤2、主軸 ≤2；舊多風格可一鍵「只留主要」 |
| **改主要（調性／主軸）** | 專案頁「改主要」列，或 Shift+點已選非主要 chip |

常數：`CHIP_SOFT_MAX`、`VISUAL_INJECT_MAX`、`LLM_INJECT_MAX`（`shared/worldview.ts`）。
