...
4. **掛 Volume**:App 服務 → Settings → Volumes → 新增,**掛載路徑填 `/data`**。
   - 沒掛 Volume 時程式會退回容器內 `./.data`,**重新部署即全部遺失**——正式站務必掛。
     系統偵測到沒掛時會亮紅警告(見下方「素材為什麼會不見、系統現在怎麼擋」),但警告救不回檔案。
...
| `ASSET_DIR` | `/data`(是真正掛載點時) | 素材儲存根目錄;Volume 掛在別處時指過去,否則檔案會寫進容器磁碟、重佈即消失。 |
| `ASSET_MAX_MB` | 200 | 單檔上限(MB),上傳與生成成品落地共用;Volume 擴容後調大即可,免改碼。 |
| `ASSET_PERSISTENT` | (自動偵測) | 覆寫持久性判定:`1`=我確定這路徑是持久卷、`0`=強制當成非持久。只有「明明掛了卷卻被誤判亮紅」時才設 1,不要拿它消警告。 |
| `ASSET_STRICT` | (無) | `1`=偵測到儲存非持久/被換卷/不可寫時**直接拒收新素材**。預設只警告不攔;詳見下方「素材為什麼會不見」一節。 |
| `ASSET_LAND_MAX_ATTEMPTS` | (內建預設) | 生成成品抓回 Volume 失敗的最大重試次數,超過即標記失敗、退出補抓佇列。 |
| `ADMIN_BACKUP_TOKEN` | (無) | 素材備份端點 `/api/admin/backup/assets.tar.gz` 的存取權杖,給外部排程 curl 用。**等同整包素材下載權**,只放 Variables、不進 git;沒設=只能在系統內人工下載備份。 |
...
| `E2E_MOCK` | `1`=全站假生成(僅供 CI/本機自動化測試)。正式站設了=夥伴以為在真生成,實際拿到假素材。 |
...
素材(上傳的檔案、AI 生成的成品)的實體檔存在磁碟上,資料庫只記「檔案放在哪個路徑」。
在雲端平台上,App 容器自己的磁碟是**用完即丟**的——每次重新部署都換一顆全新的。
所以素材消失的案例,幾乎都是同一個原因:檔案寫進了容器磁碟,而不是掛載的 Volume。
系統現在有四道防線,但請先記住第一條:

1. **掛 Volume 到 `/data` 仍然是你的必要步驟(開站步驟 4)。**
   防線只能「發現沒掛、大聲警告」,不能替你把資料變持久;警告亮起之前寫入的檔案救不回來。

2. **真掛載點偵測。**系統現在會判斷 `/data` 是不是「真正掛上來的磁碟」,而不是只看目錄存不存在
   ——容器映像裡本來就帶著一個空的 `/data` 目錄,只看「存在」等於永遠誤判成安全,這正是舊版
   假綠燈的原因。沒掛 Volume 時:`/api/ready` 亮紅、系統自檢的「儲存層(Volume)」亮紅、
   全站(包含一般組員)出現紅色警示橫幅,提醒大家先把素材備份下來、不要刪手上的原始檔。

3. **卷指紋。**系統會在 Volume 裡放一個識別指紋,每次開機比對。哪天開機發現掛到的是另一顆磁碟
   (卷被刪掉重建、平台端誤操作、還原時換了新卷),或卷突然被清空,系統會偵測到並通知開發者,
   同時亮出紅色橫幅。確認狀況無誤(例如確實是你刻意換的新卷、檔案也已經還原回去)之後,
   開發者在橫幅上按**「我已了解,這是我刻意換的新卷」**重設指紋,警示才會解除。

4. **嚴格模式(可選)。**設環境變數 `ASSET_STRICT=1`,系統在偵測到儲存非持久/被換卷/不可寫時
   會**直接拒收新素材**——上傳與生成成品落地都被擋下,並用中文告訴使用者現在不能存、該找誰處理。
   預設沒有開(只警告、不攔):因為萬一偵測誤判,攔下來等於讓一個正常站台變成唯讀。
   建議上線觀察幾天、確認偵測在你的環境判斷正確後,很在意資料的正式站把它設成 1。

就算四道防線都是綠的,**備份仍是唯一真正的還原手段**——照《維運手冊》第二章把兩件事設起來:
資料庫備份(Zeabur PostgreSQL 服務的 Backups),以及素材備份(團隊管理頁「立即下載素材備份」,
或用 `ADMIN_BACKUP_TOKEN` 讓外部排程定期抓 `/api/admin/backup/assets.tar.gz`)。

## 部署後驗收清單

1. `/api/ready` → `ok: true`、`db` 正常、`authMode: normal`。
2. 管理員登入 → 團隊管理「跑系統自檢」全綠(含儲存層寫入測試、AUTH_MODE 警示)。
   「素材備份新鮮度」在第一次備份前必然亮紅——照第 6 點處理即可,不是故障。
3. 上傳一張圖到任一專案素材庫 → Redeploy → 圖仍在(驗 Volume 掛對)。
4. 建一筆生成(真實呼叫 fal,會扣點)→ 成品可預覽、可進交付包(驗 APP_URL 基底與 FAL_KEY)。
5. 兩個瀏覽器開同一專案 → 協作游標互見(驗 WebSocket Origin 白名單)。
6. 團隊管理按一次「立即下載素材備份」(把 assets.tar.gz 存到平台以外的地方)→ 再跑一次系統自檢,
   新增的三項——**「素材備份新鮮度」「素材檔抽樣完整性」「未落地素材」——全綠**。
