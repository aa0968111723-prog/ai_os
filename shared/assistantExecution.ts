...
/** Create / update a character card — including same-name appearance rewrite. */
export const ADD_CHARACTER_INTENT_RE =
  /(?:新增|建立|加).{0,12}(?:角色|定裝)(?:卡)?|(?:角色卡|定裝卡).{0,12}(?:新增|建立|加入)|(?:改|更新|改定裝|改外觀).{0,20}(?:角色|定裝|小華|外觀)|(?:小華).{0,40}(?:粉橘|短髮|外觀|定裝|年輕男性|女孩|女生|女大生|化工|帽T|他|她)|(?:粉橘短髮|年輕男性).{0,16}(?:小華|角色)|(?:角色).{0,12}小華.{0,24}(?:外觀|定裝|粉橘|短髮|女)/i;

export function isAddCharacterIntent(message: string): boolean {
  return ADD_CHARACTER_INTENT_RE.test(message);
}
...
const CAPABILITY_GOAL_PATTERNS: ReadonlyArray<{ id: string; pattern: RegExp }> = [
  ...
  { id: "add_character", pattern: ADD_CHARACTER_INTENT_RE },
