...
export const MCP_TOOLS: McpToolInfo[] = [
  ...
  // CUTOS 影片剪輯（治理後暴露；apply／export／undo／redo 一律不開放給 MCP，
  // 因為 MCP 沒有人工核准介面，開放等於繞過核准閘）
  { name: "cutos_get_project", title: "查影片專案", access: "read", blurb: "查專案連結的 CUTOS 影片狀態與時間軸版本。" },
  { name: "cutos_get_transcript", title: "讀逐字稿", access: "read", blurb: "分頁讀取影片逐字稿。" },
  { name: "cutos_search_transcript", title: "搜尋逐字稿", access: "read", blurb: "在逐字稿中做字面搜尋。" },
  { name: "cutos_search_semantic", title: "語意搜尋影片", access: "read", blurb: "找出影片中談到某個主題的片段。" },
  { name: "cutos_list_speakers", title: "列出說話者", access: "read", blurb: "列出影片中的說話者與發言時間。" },
  { name: "cutos_list_topics", title: "列出影片主題", access: "read", blurb: "列出影片談到的主題與時間範圍。" },
  { name: "cutos_find_highlights", title: "找精華片段", access: "read", blurb: "找出適合做精華或短影音的片段。" },
  { name: "cutos_inspect_scene", title: "檢視影片片段", access: "read", blurb: "檢視某段時間區間的內容摘要。" },
  { name: "cutos_get_preview", title: "查即時預覽", access: "read", blurb: "取得目前時間軸的即時預覽資訊。" },
  { name: "cutos_get_job", title: "查影片工作", access: "read", blurb: "查 CUTOS 背景工作（分析／輸出）的進度。" },
  { name: "cutos_verify_edit_plan", title: "驗證剪輯計畫", access: "read", blurb: "驗證待審核的剪輯計畫是否可安全套用。" },
  { name: "cutos_list_activity", title: "查影片活動", access: "read", blurb: "查影片分析／剪輯的跨系統活動事件。" },
  { name: "cutos_start_analysis", title: "開始分析影片", access: "write", blurb: "送出影片分析工作（不改動時間軸）。", openWorld: true, idempotent: true },
  { name: "cutos_create_edit_plan", title: "建立剪輯計畫", access: "write", blurb: "以自然語言建立待審核的剪輯計畫；仍需人工核准才會套用。", openWorld: true, idempotent: true },
  ...
  { name: "list_knowledge", title: "列出知識庫", access: "read", blurb: "列出專案知識庫（腳本／逐字稿／見證／筆記）的摘要與字數；全文用 get_knowledge。" },
  { name: "get_knowledge", title: "讀取知識", access: "read", blurb: "分段讀一筆知識全文（每次最多 20000 字，offset 續讀）。" },
  { name: "list_scenes", title: "列出分鏡", access: "read", blurb: "列出專案分鏡（shot）的順序、標題、狀態與畫面／配音齊備度（唯讀摘要）。" },
  { name: "list_tasks", title: "列出人類任務", access: "read", blurb: "列出專案的人員任務與核准請求（狀態／負責人／期限／來源計畫）。" },
  ...
  { name: "update_scene", title: "更新分鏡", access: "write", idempotent: true, blurb: "更新分鏡（shot）標題、提示詞、旁白、環境音或剪輯（秒數與素材入出點）。" },
