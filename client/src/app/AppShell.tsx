...
export function AppShell() {
  ...
  const [location, navigate] = useLocation();
  /**
   * 首屏只打一支 bootstrap（伺服器聚合 me + 未讀 + mock）。
   * 裝置不再自己排三支 API；輪詢也只打這一支，由伺服器重算未讀。
   */
  const boot = trpc.auth.bootstrap.useQuery(undefined, {
    staleTime: 60_000,
    refetchOnWindowFocus: false,
    refetchInterval: (q) => (q.state.data?.me ? 60_000 : false),
  });
  // 相容既有變數名：下游仍用 me / info / dmUnread
  const me = {
  ...
  const logout = trpc.auth.logout.useMutation({
    onSuccess: () => {
      void utils.auth.bootstrap.invalidate();
