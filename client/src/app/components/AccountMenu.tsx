 ...
 *
 * 手機（<768px）少列「說明／工作」兩組——那裡底部分頁列的「更多」面板就是全站頁面總表，
      ...
      <MenuSurface open={open} onClose={close} label="使用者選單" triggerRef={triggerRef} className="account-menu__menu">
          ...
          {compact ? (
            ...
            <div className="menu-note" role="presentation">
              {/* 點名的頁面必須真的在「更多」面板裡（mobileMoreGroups），否則就是叫人
                  去找不存在的東西。資料中心一度被拿出面板、這句也跟著拿掉它，結果手機上
                  三個選單都沒有入口——現在兩邊一起補回來。 */}
              <Hint as="span">要去其他頁面（資料中心、靈感頻道、動畫創作室、怎麼用…）請按最底下的「更多」</Hint>
            </div>
          ) : (
            <>
