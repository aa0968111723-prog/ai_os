...
export function UngroupedRoutes() {
  return (
    <Switch>
      ...
      <Route path="/chat"><ChatPage /></Route>
      <Route>
        <EmptyState title={<>你已成功加入 ✓ 還差一步</>} description={<>帳號建立完成，只是還沒被分進任何組別。請聯絡你的組長或管理員把你加入組——加入後重新整理這一頁，就能開始創作。</>} action={<><p className="hint">等待的時候可以先<Link href="/help">看看怎麼用</Link>，了解點數、生成與審核是怎麼運作的。</p></>} style={{ marginTop: "var(--sp-32)" }} />
      </Route>
    </Switch>
  );
}
