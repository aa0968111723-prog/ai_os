...
export function ShareTargetPage({ groupId }: { groupId: string }) {
  ...
  useEffect(() => {
    void readShareInbox().then((got) => {
      ...
      if (got && got.files.length === 0) setDest("dm");
    });
  }, []);
    ...
    <div className="page-shell secondary-page share-target-page">
      ...
      {inbox === "loading" ? (
        ...
        <EmptyState
          ...
          title={<>目前沒有待存的分享</>}
          description={<>在手機相簿或其他 App 選檔案 → 按「分享」→ 選「Aios」，內容就會出現在這裡。（需要先把 Aios 安裝到主畫面）</>}
        ...
        <div className="stack">
          ...
          <Card as="section" aria-label="選擇去處">
            ...
            {files.length === 0 && (
              <Hint layer="always">純文字／連結的分享只能傳給夥伴；要進素材庫請分享檔案。</Hint>
            )}
            ...
            <div className="share-target-actions">
              <Button variant="primary" disabled={!canSave} onClick={() => void submit()}>
                {busy ? (step || "處理中…") : dest === "project" ? "存進素材庫" : "傳送私訊"}
              </Button>
              <Button
                variant="ghost"
                disabled={busy}
