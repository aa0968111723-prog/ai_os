...
const h = vi.hoisted(() => {
  const bag: {
    ...
    /** mutation 的錯誤（依 tRPC 路徑登記）：存檔失敗後畫面有沒有把失敗講出來，只有這樣才測得到。
     *  固定 error: null 的話，「使用者以為權限改好了、其實後端沒收下」這一類問題一條都驗不了。 */
    mutationError: Map<string, { message: string }>;
    /** 查詢回傳值與查詢的失敗狀態（依 tRPC 路徑登記）：整頁的測試要靠它餵出通訊錄與指揮權現況 */
    queryData: Map<string, unknown>;
    queryError: Map<string, { message: string }>;
  } = {
    mutations: [],
    root: {},
    mutationError: new Map(),
    queryData: new Map(),
    queryError: new Map(),
  };
  ...
  const makeNode = (path: string): Record<string, unknown> => {
    const base: Record<string, unknown> = {
      useQuery: () => {
        const error = bag.queryError.get(path) ?? null;
        return { data: bag.queryData.get(path), isLoading: false, isError: !!error, error };
      },
...
import { COMMAND_LEVEL_OPTIONS, CommandLevelField, describeCommandLevel, MembersPage } from "./MembersPage";
...
describe("CommandLevelField", () => {
  ...
  it("存檔失敗要朗讀得出來，並接到選單上——焦點還在選單，聽不到就會以為權限已經改好了", () => {
    h.mutationError.set("quota.setMemberCommandLevel", { message: "只有團隊管理員以上可以調整指揮權" });
    render(<CommandLevelField groupId={GROUP} userId={USER} level="none" />);
    const select = screen.getByLabelText("組代理指揮權");
    const alert = screen.getByRole("alert");
    expect(alert).toHaveTextContent("只有團隊管理員以上可以調整指揮權");
    // aria-describedby 要真的指到那句話：指到不存在的 id 等於沒接，讀螢幕的人一樣聽不到
    expect(select).toHaveAttribute("aria-invalid", "true");
    expect(select.getAttribute("aria-describedby")).toBe(alert.id);
    expect(alert.id).toBeTruthy();
  });

  it("沒有錯誤時不掛 aria-invalid（永遠掛著的話，「這一格有問題」就變成沒有意義的雜訊）", () => {
    render(<CommandLevelField groupId={GROUP} userId={USER} level="none" />);
    const select = screen.getByLabelText("組代理指揮權");
...
describe("指揮權現況讀不到時，通訊錄不可以安靜地少一塊", () => {
  /** 一個組、一位組員：這樣 focusGroupId 自動落在唯一管得到的那個組，指揮權那一段才會出現 */
  function seedDirectory() {
    h.queryData.set("directory.scope", { groups: [{ groupId: GROUP, teamName: "行銷", groupName: "影音組" }] });
    h.queryData.set("auth.me", { user: { id: "me" } });
    h.queryData.set("directory.list", {
      members: [{
        userId: USER, name: "阿光", email: "a@example.com", isSuperAdmin: false, disabled: false,
        lastLoginAt: null, lastActionAt: null, lastActionLabel: null,
        memberships: [{
          groupId: GROUP, teamName: "行銷", groupName: "影音組", role: "member",
          weeklyUsed: 0, totalUsed: 0, budget: null,
        }],
      }],
    });
  }

  it("quota.usage 失敗時那句話要朗讀得出來——不然組長會以為這個組根本沒有指揮權設定", () => {
    seedDirectory();
    h.queryError.set("quota.usage", { message: "連線逾時" });
    render(<MembersPage />);
    // 通訊錄本身載入成功，所以整頁只有這一個 alert：就是「現況讀不到」那句
    expect(screen.getByRole("alert")).toHaveTextContent("指揮權現況載入失敗：連線逾時");
    // 讀不到現況就不畫控制項——寧可少一個入口，也不要送出一個猜出來的等級
    expect(screen.queryByLabelText("組代理指揮權")).toBeNull();
  });
});
