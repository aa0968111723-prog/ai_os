...
import type { VisualChoiceFamily } from "@shared/visualChoiceTypes";
...
export function StoryboardStage({
  ...
  const [visualFamily, setVisualFamily] = useState<VisualChoiceFamily | null>(null);
  const [visualForceOpen, setVisualForceOpen] = useState(false);
  ...
  const ensurePicked = useCallback((shotId: string) => {
    setPicked((prev) => {
  ...
  const openVisualChoice = useCallback(
    (family: VisualChoiceFamily, shotId: string) => {
      ensurePicked(shotId);
      setVisualFamily(family);
      setVisualForceOpen(true);
    },
    [ensurePicked],
  );
      ...
      <div className="board-stage__layout" style={{ display: "flex", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
          ...
          {isEmpty ? (
            ...
            <>
              <Hint style={{ margin: "4px 0 10px" }}>
                點畫面或「單格工作室」細修；右側「資源」可就近取用素材／定裝。分鏡卡上的「視覺選擇」或勾選多鏡後，可一次套用動作／表情／鏡頭／光線。
              </Hint>
                        ...
                        <div className="board-shots">
                          {group.shots.map((shot) => (
                            <ShotCard
                              ...
                              onOpenVisualChoice={canEdit ? openVisualChoice : undefined}
                            />
                          ))}
                        </div>
                      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, flex: "0 0 auto" }}>
          ...
          <VisualChoiceTray
            projectId={projectId}
            canEdit={canEdit}
            pickedShotIds={pickedIds}
            initialFamily={visualFamily}
            forceOpen={visualForceOpen}
            onOpened={() => setVisualForceOpen(false)}
          />
        </div>
      </div>
      {studioShot && (
