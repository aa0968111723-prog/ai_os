...
export function ShotCard({
  ...
  /** 打開右側 VisualChoiceTray 並切到指定 family（會先勾選本鏡） */
  onOpenVisualChoice?: (family: VisualChoiceFamily, shotId: string) => void;
}) {
    ...
    >
      ...
      <div className="shot-card__face">
        ...
        <div className="shot-card__face-actions">
          ...
          {canEdit && onOpenVisualChoice && (
            <Button
              size="sm"
              variant="tonal"
              type="button"
              title="打開視覺選擇：動作／表情／鏡頭／光線"
              onClick={(e) => {
                e.stopPropagation();
                onOpenVisualChoice("action", shot.id);
              }}
            >
              <Icon name="Sparkles" size={13} /> 視覺選擇
            </Button>
          )}
