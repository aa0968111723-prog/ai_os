...
export function AssetLibrary({
  ...
  const shown = useMemo(() => {
    let list = allAssets ?? [];
    if (revisionOfId) {
      list = listDirectRevisions(list, revisionOfId);
    }
                          ...
                          <div className="asset-lineage" style={{ marginTop: 4 }}>
                            {lineage && (
                              <Meta as="div" style={{ fontSize: 11 }} title={m.sourceAssetId}>
                                <Icon name="Clapperboard" size={11} style={{ verticalAlign: "-1px", marginRight: 4 }} />
                                {lineage}
                                {m.sourceAssetId && (
                                  <button
                                    type="button"
