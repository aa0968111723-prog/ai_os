 ...
 * - BYOK Phase 2：falSubmit / falStatus 接受 optional apiKey；未傳則用 process.env.FAL_KEY。
 */
...
export type FalKeyOpts = {
  /** BYOK：個人 API Key。未傳則用 process.env.FAL_KEY。submit 與 status 必須用同一把。 */
  apiKey?: string;
};

function resolveFalKey(opts?: FalKeyOpts): string {
  const key = (opts?.apiKey?.trim() || process.env.FAL_KEY || "").trim();
  if (!key) {
    throw new Error("FAL_KEY 未設定：請到部署平台的服務 Variables 填入 fal.ai 金鑰後重新部署（或於 BYOK 設定個人金鑰）");
  }
  return key;
}
