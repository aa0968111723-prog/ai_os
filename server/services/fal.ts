 ...
 * - BYOK Phase 2：falSubmit / falStatus 接受 opts.apiKey（個人金鑰）；未傳則 fallback 站方 FAL_KEY。
 */
 ...
 *
 * 注意：BYOK 個人金鑰路徑的「略過點數」由 generationCore 依 usedUserKey 決定，
 * 不經過此函式（此函式只服務 e2e mock）。
 */
export function billingBypassed(): boolean {
  return MOCK && process.env.MOCK_BILLING !== "1";
}

/** BYOK：解析實際要用的金鑰。優先 opts.apiKey（個人），否則站方 FAL_KEY。 */
function resolveFalKey(apiKey?: string): string {
  const key = (apiKey ?? process.env.FAL_KEY ?? "").trim();
  if (!key) {
...
export type FalRequestOpts = {
  /** BYOK：個人 fal API Key；未傳則用站方 FAL_KEY */
  apiKey?: string;
};
...
export async function falSubmit(
  ...
  const key = resolveFalKey(opts?.apiKey);
