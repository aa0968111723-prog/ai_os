...
async function runTool(auth: AuthState, scope: McpScope, name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === "list_schedule" || name === "add_schedule_item" || name === "get_project_status") {
    ...
    if (name === "list_schedule") {
      const items = await listScheduleForGroup(auth, project.groupId, Boolean(args.includePast));
      // 只回「與本專案相關」：綁本專案的 ＋ 組層級（未綁專案）的行程
      return items
        .filter((i) => i.projectId === project.id || i.projectId == null)
    ...
    const sched = await listScheduleForGroup(auth, project.groupId, false);
