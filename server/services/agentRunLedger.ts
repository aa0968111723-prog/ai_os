...
export class AgentRunLedger implements RunLedger {
  ...
  async save(run: DurableRun): Promise<void> { await db.update(schema.agentRuns).set({ steps: run.steps, status: run.status === "completed" ? "done" : run.status, updatedAt: new Date(run.updatedAt) }).where(eq(schema.agentRuns.id, run.id)); }
