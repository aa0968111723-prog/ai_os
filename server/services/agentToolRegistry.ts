...
async function authFor(context: ToolContext) {
  ...
  if (!auth || !auth.groups.some((membership) => membership.groupId === context.groupId)) throw new Error("FORBIDDEN");
  return auth;
}
