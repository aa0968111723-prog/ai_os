...
// ── 個資自助匯出（深度版）：帳號／組／專案深摘要／生成／帳本／代理／咒語／審批／私訊…
// 預設 HTML 可讀；?format=json 原始結構。絕不含 passwordHash。
app.get("/api/me/export", async (req, res) => {
  try {
    const auth = await resolveSession(req);
    if (!requireUsableSession(auth, res)) return;
    const { loadMyDataExportPayload } = await import("./services/myDataExportLoad");
    const payload = await loadMyDataExportPayload(auth);
