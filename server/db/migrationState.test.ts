...
describe("legacy migration adoption bridge", () => {
  ...
  it("accepts reviewed DDL the legacy database already carries when it re-runs as a no-op", () => {
    ...
    // 8 項來自 bridge 前綴；另外 11 項來自 bridge 之後的 0036／0037／0038／0039／0040／0041——這份
    // 合成 drift 只放了 bridge 的語句，那幾支的最終形狀自然不在裡面。它們都寫了 IF NOT EXISTS，
    ...
    //   0041 是 content_attachments 的建表與兩個索引，三句。）
    //
