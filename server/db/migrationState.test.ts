...
describe("legacy migration adoption bridge", () => {
  ...
  it("accepts reviewed DDL the legacy database already carries when it re-runs as a no-op", () => {
    ...
    // 8 項來自 bridge 前綴；另外 11 項來自 bridge 之後的 0036／0037／0038／0039／0040——這份合成
    // drift 只放了 bridge 的語句，那幾支的最終形狀自然不在裡面。它們都寫了 IF NOT EXISTS，
    // 重跑是 no-op，所以同樣計入 alreadyPresent 而不是判成缺漏。
    //（0038 是 scenes 的兩個環境音欄位，一支 migration 兩句 ADD COLUMN；
    //   0039 是 project_share_links 的建表與建索引，同樣兩句，故合計 +4 而非 +2；
    //   0040 是 community_posts 的三個分類欄位＋兩個索引，五句皆 IF NOT EXISTS，故再 +5。）
    //
    // 這個數字刻意寫死、不動態算：新增一支 migration 就要有人回來改這一行，
    // 而改之前得先確認新語句真的是「重跑無害」——動態計算會讓非冪等的 DDL 悄悄溜過去。
    expect(result.alreadyPresent).toBe(8 + 11);
  });
