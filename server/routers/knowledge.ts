...
export async function buildKnowledgeContextWithMeta(
  ...
  // 附件文字（0043）：知識庫收得進 PDF／Word 之後，那份檔案的內文才是使用者心目中的「素材」。
...
export const knowledgeRouter = router({
  ...
  addFromAsset: authedProcedure.input(z.object({ assetId: z.string().uuid() })).mutation(async ({ ctx, input }) => {
    ...
    const job = (async () => {
      ...
      const { readStoredHead } = await import("../services/storage");
      let content: string;
      try {
        ...
        // readStoredHead 兩種儲存後端通用（物件儲存走 Range，一樣不會整檔拉下來）。
        const buf = await readStoredHead(asset.storagePath, MAX_CONTENT * 4);
        content = buf.toString("utf8").slice(0, MAX_CONTENT);
      } catch {
