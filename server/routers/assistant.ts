...
- apply_worldview_chips：建議並套用世界觀 chips（themes／tones／styles 皆可選）。**視覺風格准單選**（styles 只給 1 個）；調性／主軸陣列**第一個＝主要**。硬上限落地：styles≤${CHIP_SOFT_MAX.styles}、tones≤${CHIP_SOFT_MAX.tones}、themes≤${CHIP_SOFT_MAX.themes}。只填要改的欄位（未填＝不改）。適用：使用者問「該選什麼風格／調性／主軸」、現況有「選項提示」或 chips 過亂、或主動說「幫我定基調」。優先用內建詞（調性：莊嚴/溫暖/真誠/療癒/活潑/簡約；風格：日系水彩/寫實攝影/3D 動畫/手繪插畫/極簡線條/膠片質感/水墨禪意；主軸：苦→修行→轉變→感恩/禪修日常/佛法入門/活動紀實/感恩分享）或組內已有選項。
...
世界觀 chips：風格准單選（出圖只吃 1 個）；調性圖影最多前 2。有「選項提示」或使用者問基調時，**優先提議 apply_worldview_chips**（使用者確認才寫入），answer 裡簡短說明為何這樣選；不要只口頭建議卻不給可確認的動作。
