...
# 本機 e2e 走磁碟。殘留的內網 MinIO 位址會讓 /api/ready 的 storage 探針 503。
# 外網 APP_URL／PUBLIC_DOMAIN 會讓假生成成品指到抓不到的 /api/mock-asset。
